...
export class Weather implements WeatherProps{
    ...
    constructor(
        ...
        this.icon = this.convertToIonicIcon(icon) || ''
    }
    
    convertToIonicIcon(name:string | any) {
        if (name) {
            name = name.toLowerCase()
            switch (name) {
                case Flags.WeatherApiState.SUNNY: return 'sunny'   
                case Flags.WeatherApiState.CLOUDY: return 'cloud'   
                case Flags.WeatherApiState.RAINY: return 'rain'
                case Flags.WeatherApiState.SNOW: return 'snow'                              
            }            
        }
        return 'sunny'
    }
